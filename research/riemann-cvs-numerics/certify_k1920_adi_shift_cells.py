#!/usr/bin/env python3
"""Certify the concrete K=1920 ADI root/pole noncollision cells.

The rank-86 adjacent-shell certificate uses 31 same-sign and 12 reflected
Zolotarev/ADI factors.  The generic Lean telescoping theorem is exact once no
source or target grid point meets an ADI root or pole.  For K=1920 most of the
noncollision facts follow from the coarse signs/ranges
...
strict integer cell enclosure with Arb,
    ...
    m < 1920 * pole < m+1                  (same-sign),
    m < 1920 * (-pole) < m+1               (reflected).
...
separate inverse cross-ratio formula; all 86 root/pole differences must contain
zero.  Thus the artifact audits both the grid cells and the shift construction,
while Lean's ``K1920AdiShiftBinding`` module turns precisely these inequalities
into the noncollision premises of the exact ADI factorization theorem.
"""
...
import flint
from certify_k1920_adjacent_compressed_gram import _roots_and_poles
...
def _progress(message: str) -> None:
    print(f"[k1920-adi-shift-cells] {message}", flush=True)
...
def certify(
    *, precision: int, threads: int, reference_json: Path | None
) -> dict[str, Any]:
    if precision < 128:
    ...
    _progress("certifying 31 same-sign root/pole ranges and target-pole cells")
    ...
    _progress("certifying 12 reflected root/pole ranges and target-pole cells")
    ...
    production_path = Path(
        __import__("certify_k1920_adjacent_compressed_gram").__file__
    ).resolve()
    fraction_path = Path(
    ...
    payload: dict[str, Any] = {
        ...
        "scope": "literal K=1920 ADI shift ranges and pole grid noncollision cells",
        ...
        "lean_target": "RiemannCvs.K1920AdiShiftBinding",
        "proof_boundary": (
...
def main() -> int:
    ...
    print(
        "K1920 ADI shift-cell certificate PASS: "
